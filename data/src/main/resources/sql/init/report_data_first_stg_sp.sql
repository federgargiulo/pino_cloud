...
BEGIN
     INSERT INTO procedure_execution_log (procedure_name, start_time, status, message)
        ...
        INSERT INTO report_data_first_stg (id , tenant_id, equipment_id, signal_id, reference_timestamp,
                                   year_val, month_val, day_val, hour_val,
								   minute_val,
                                   mean_val, min_val, max_val, count_of_measure)
        SELECT
			nextval('report_data_first_stg_seq'),
            tenant_id,
            equipment_id,
            signal_id,
           date_trunc('hours', measure_dttm) +
                 FLOOR( EXTRACT(minute FROM measure_dttm) / 10 ) * 10 * INTERVAL '1 minute'  AS reference_timestamp,
            year_val,
            month_val,
            day_val,
            hour_val,
            (minute_val / 10::int) * 10 AS minute_round,
            AVG(val::NUMERIC),
            MIN(val::NUMERIC),
            MAX(val::NUMERIC),
            COUNT(*)
        ...
        WHERE
			date_trunc('hours', measure_dttm) +
                 FLOOR( EXTRACT(minute FROM measure_dttm) / 10 ) * 10 * INTERVAL '1 minute'
				 <=
				 date_trunc('minute', ( start_time- INTERVAL '1 hour' * interval_hours ) ) +
                FLOOR( EXTRACT(minute FROM start_time ) / 10 ) * 10 * INTERVAL '1 minute'
